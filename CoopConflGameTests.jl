...
params = SimulationParameters(action0=0.1, a0=2.0, p0=0.1, population_size=10, relatedness=1.0, inflation_factor=8)  # uses all default values
pop = population_construction(params);

update_norm_punishment_pools!(pop)
pairs, num_pairs = shuffle_and_pair(collect(keys(pop.individuals)), pop.parameters.population_size, pop.parameters.relatedness)

u0s, ps = collect_initial_conditions_and_parameters(pairs, num_pairs, pop)
@time final_actions = behav_eq(u0s, ps, pop.parameters.tmax, num_pairs)
